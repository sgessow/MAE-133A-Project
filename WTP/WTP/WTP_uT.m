...
if T <= 623.15
    ...
    if u < uf
        [Pf,Tf,vf,u_test,sf,hf] = property_PT(100,T,1); 
    ...
    if u > ug
     ...
     [Pf,Tf,vf,u_test2,sf,hf] = property_PT(0.000001,T,2); 
    if u > u_test2
...
if T < 273.15001 | u<0  % basic quality control
    state=5;
end
...
if state==1 % T < Ts, Liquide comprimé
    ...
    if u < u_test 
        ...
        while abs(u_est-u)>tol
            Pavg=0.5*(Pmin+Pmax);
            [Pf,Tf,vf,u_est,sf,hf] = property_PT(Pavg,T,1); 
            if u_est < u
                Pmax=Pavg;
            else
                Pmin=Pavg;
...
if state==2  % T > Ts, Vapeur surchauffé
    ...
    if T > 1073.15 | P >100
        ...
        [Pg,Tg,vf,u_min,sg,hg] = property_PT(Pmin,T,2);   % verify that u supplied is consistent
        ...
        if u > u_min
            ...
            while abs(u_est-u)>tol
                Pavg=0.5*(Pmin+Pmax);
                [Pg,Tg,vf,u_est,sg,hg] = property_PT(Pavg,T,2); 
                if u_est < u
                    Pmax=Pavg;
                else
                    Pmin=Pavg;
...
if state==5  % impossible data
    ...
    info_msg_2= 'P or T < 0, P too small, or impossible combo'
end
