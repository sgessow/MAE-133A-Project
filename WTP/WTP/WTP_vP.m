...
if state==1 %  Liquide comprimé
    ...
    % verify volume is within the bounds
           [Pf,Tf,vt1,uf,sf,hf] = property_PT(P,Tmin,1); 
           [Pf,Tf,vt2,uf,sf,hf] = property_PT(P,Tmax,1); 
    if v < vt2 & v >vt1       
        tol=0.0000001;
        v_est=100000000;
        while abs(v_est-v)>tol
            Tavg=0.5*(Tmin+Tmax);
            [Pf,Tf,v_est,uf,sf,hf] = property_PT(P,Tavg,1); 
            if v_est < v
                Tmin=Tavg;
            else
                Tmax=Tavg;
...
if state==2  % T > Ts, Vapeur surchauffé
    ...
    if T > 1073.15 | P >100
        ...
        while abs(v_est-v)>tol
            Tavg=0.5*(Tmin+Tmax);
            [Pg,Tg,v_est,ug,sg,hg] = property_PT(P,Tavg,2); 
            if v_est < v
                Tmin=Tavg;
            else
                Tmax=Tavg;
...
if state==4  % impossible data
    ...
    info_msg_2= 'or P > 100000 kPa or T > 800C'
end
...
if state==6  % impossible data
    info_msg = 'Incorrect data'
    info_msg_2= 'v < 0.00001 m/kg or impossible combo'
end
